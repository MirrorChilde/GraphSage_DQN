...
import random
from GraphSage import model as gnn
...
class DQNAgent:
	def __init__(self, batch_size):
		...
		self.global_step = 0
		# self.primary_network = gnn.myModel(out_dim=[64,hparams['link_state_dim']],aggr_method='pooling',activation_rounds=['selu','softmax'],use_bias=True,sample_nums=[6,3])
		# self.primary_network.compile(loss=loss_fn, optimizer=self.optimizer, metrics=['acc'])
		# self.primary_network.compile(loss=loss_fn, optimizer=self.optimizer, metrics=[accuracy_fn])

		# self.target_network = gnn.myModel(out_dim=[64,hparams['link_state_dim']],aggr_method='pooling',activation_rounds=['selu','softmax'],use_bias=True,sample_nums=[6,3])
		# self.target_network.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['acc'])
		# self.primary_network.compile(loss=loss_fn, optimizer=self.optimizer, metrics=[accuracy_fn])
...
if __name__ == "__main__":
	...
	for ep_it in range(ITERATIONS):
		...
		save_reward('DQN_GraphSage-' + str(graph_topology), ep_it, evalMeanReward)
		# if ep_it%5==0:
