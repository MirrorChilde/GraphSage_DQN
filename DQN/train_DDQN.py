...
class DQNAgent:
    def __init__(self, batch_size):
        ...
        self.current_network = gnn.myModel(hparams)
        self.current_network.build()
    ...
    def act(self, env, state, demand, source, destination, flagEvaluation):
        """
        给定一个存储在环境中的需求它在当前状态下分配K=4条最短路径
        并利用GNN模型预测K=4种不同新图态的q_value。
        根据贪心方法选择状态。flag=TRUE表示我们正在测试
        模型，因此，它不会激活下降层。
        """
        # 如果我们需要计算K=4个q值并取最大值，则设置为True
        takeMax_epsilon = False
        # List of graphs
        listGraphs = []
        # 在cummax()调用中使用的图形特性列表
        ...
        # 我们计算graphs_ids，以便稍后对每个图执行unsorted_segment_sum，并获得
        # 链接每个图的隐藏状态。
    ...
    def _forward_pass(self, x):
        prediction_state = self.primary_network(x[0], x[1], x[2], x[3], x[4], training=True)  # 取出所选动作的Q值
        prediction_next_current = tf.stop_gradient(
            self.target_network(x[13], x[14], x[16], x[17], x[18], training=True))  # 取出动作
        preds_next_target = tf.stop_gradient(
            self.target_network(x[6], x[7], x[9], x[10], x[11], training=True))  # 计算下一个目标Q网络
        return prediction_state, preds_next_target, prediction_next_current
    ...
    def _train_step(self, batch):
        ...
        with tf.GradientTape() as tape:
            ...
            target = []  # y_batch
            for x in batch:
                prediction_state, preds_next_target, prediction_next_current = self._forward_pass(x)
                # 取所执行动作的q值
                preds_state.append(prediction_state[0])
                # 取动作
                # 如果done==TRUE，则乘以0来消去第二项
                target.append(
                    tf.stop_gradient([x[5] + self.gamma * tf.math.reduce_max(prediction_next_current) * (1 - x[8])]))
    ...
    def replay(self, episode):
        ...
        if episode % copy_weights_interval == 0:
            ...
            # self.current_network.set_weights(self.primary_network.get_weights())
        gc.collect()

    # state_action == agent.act
    def add_sample(self, env_training, state_action, reward, done, new_state, new_demand, new_source, new_destination,
                   state, demand, source, destination):
        self.bw_allocated_feature.fill(0.0)
        new_state_copy = np.copy(new_state)
        state_copy = np.copy(state)  # 修改部分
        ...
        old_pathList = env_training.allPaths[str(source) + ':' + str(destination)]

        path = 0
        path_old = 0
        list_k_features = list()
        list_k_features_old = list()
        ...
        # 修改部分
        while path_old < len(old_pathList):
            oldPath = old_pathList[path_old]
            i = 0
            j = 1

            while (j < len(oldPath)):
                state_copy[env_training.edgesDict[str(oldPath[i]) + ':' + str(oldPath[j])]][1] = demand
                i = i + 1
                j = j + 1

            old_features = agent.get_graph_features(env_training, state_copy)
            list_k_features_old.append(old_features)
        ...
        old_vs = [v for v in list_k_features_old]
        old_graph_ids = [tf.fill([tf.shape(old_vs[t]['link_state'])[0]], t) for t in range(len(list_k_features_old))]
        old_first_offset = cummax(old_vs, lambda v: v['first'])
        old_second_offset = cummax(old_vs, lambda v: v['second'])
        old_tensors = (
            {
                'graph_id': tf.concat([v for v in old_graph_ids], axis=0),
                'link_state': tf.concat([v['link_state'] for v in old_vs], axis=0),
                'first': tf.concat([v['first'] + m for v, m in zip(old_vs, old_first_offset)], axis=0),
                'second': tf.concat([v['second'] + m for v, m in zip(old_vs, old_second_offset)], axis=0),
                'num_edges': tf.math.add_n([v['num_edges'] for v in old_vs]),
            }
        )
        ...
        self.memory.append(
            (state_action['link_state'],
             ...
             tf.convert_to_tensor(reward, dtype=tf.float32),
             old_tensors['link_state'],
             old_tensors['graph_id'],  # 15
             tf.convert_to_tensor(int(done == True), dtype=tf.float32), old_tensors['first'],
             old_tensors['second'],
             # 18
             tf.convert_to_tensor(old_tensors['num_edges']))
        )
...
if __name__ == "__main__":
    ...
    counter_store_model = 1  # python DQN_mpnn_sample.py
    # Get the environment and extract the number of actions.
    env_training = gym.make(ENV_NAME)
    np.random.seed(SEED)
    env_training.seed(SEED)
    ...
    for ep_it in range(ITERATIONS):
        ...
        for _ in range(train_episodes):
            ...
            while 1:
                ...
                agent.add_sample(env_training, state_action, reward, done, new_state, new_demand, new_source,
                                 new_destination, state, demand, source, destination)
                state = new_state
                demand = new_demand
                source = new_source
        ...
        evalMeanReward = np.mean(rewards_test)
        save_throughput('DDQN-' + str(graph_topology), ep_it, evalMeanReward, demandlist, min_utilization,
                        mean_utilization)
        print(ep_it, evalMeanReward)
