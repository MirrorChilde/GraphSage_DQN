...
import random
# import mpnn as gnn
from GAT import agent as GATnetwork
...
class DQNAgent:
    def __init__(self, batch_size):
        ...
        self.global_step = 0
        self.primary_network = GATnetwork.GraphAttentionNetwork(HIDDEN_UNITS, NUM_HEADS, NUM_LAYERS, OUTPUT_DIM)
        self.primary_network.compile(loss=loss_fn, optimizer=self.optimizer, metrics=[accuracy_fn])
        self.target_network = GATnetwork.GraphAttentionNetwork(HIDDEN_UNITS, NUM_HEADS, NUM_LAYERS, OUTPUT_DIM)
        self.target_network.compile(loss=loss_fn, optimizer=self.optimizer, metrics=[accuracy_fn])
...
if __name__ == "__main__":
    ...
    for ep_it in range(ITERATIONS):
        ...
        save_reward('DQN_GAT-' + str(graph_topology), ep_it, evalMeanReward)
        # if ep_it%5==0:
